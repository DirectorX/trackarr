...
func parseTrackerServers(doc *xmlquery.Node, tracker *TrackerInfo) error {
	for _, n := range xmlquery.Find(doc, "//servers/server") {
		...
		log.Debugf("Found server hosts: %s", strings.Join(serverHosts, ", "))
		...
		log.Debugf("Found server channels: %s", strings.Join(serverChannels, ", "))
		...
		log.Debugf("Found server announcers: %s", strings.Join(serverAnnouncers, ", "))
